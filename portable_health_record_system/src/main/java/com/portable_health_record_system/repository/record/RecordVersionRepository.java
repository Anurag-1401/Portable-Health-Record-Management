...
public interface RecordVersionRepository extends JpaRepository<RecordVersion, UUID> {
    ...
    List<RecordVersion> findByRecordIdOrderByVersionNumberAsc(
            UUID recordId
    );

    List<RecordVersion> findByPatientIdOrderByCreatedAtAsc(
            UUID patientId
    );

    Optional<RecordVersion> findByRecordIdAndVersionNumber(
            UUID recordId,
            long versionNumber
    );
}